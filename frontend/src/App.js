...
export default function App() {
  ...
  // 🛑 UMAR BHAI FIX: The Ultimate Responsive Scroll
  ...
  const handleAuth = async (e) => {
    ...
    if (authMode === 'signup' && !isPasswordValid) { setAuthError("Please meet requirements."); return; }
    setAuthLoading(true);
    try {
      ...
      if (res.ok) {
          ...
          else if (authMode === 'otp') { setAuthMode('login'); setAuthSuccess('✅ Verified!'); setOtp(''); setPassword(''); }
          else {
              setUser({ email });
              localStorage.setItem('al_elite_user_final3', JSON.stringify({ email }));
              setShowAuthModal(false);
              setMessages([{ role: 'ai', text: `Welcome back, ${email.split('@')[0]}!`, id: Date.now() }]);
          }
      } else { setAuthError(data.detail || "Failed"); }
    } catch { setAuthError("⚠️ Connection Error."); }
    setAuthLoading(false);
  };

  const logout = () => {
      setUser(null); localStorage.removeItem('al_elite_user_final3'); setChatHistory([]); setMessages([defaultWelcomeMessage]); setShowProfileSettings(false);
  };

  // --- 🚀 CHAT LOGIC ---
  const startNewChat = () => {
    setMessages([{ role: 'ai', text: user ? "New session started." : "Hello! I'm your Advisor.", id: Date.now() }]);
    setActiveChatId(null); setIsTempChat(false); setShowTempDisclaimer(false); setIsSidebarOpen(false);
  };

  const toggleTempChat = () => {
    if (!isTempChat) { setShowTempDisclaimer(true); setMessages([{ role: 'ai', text: "🕶️ Incognito Mode.", id: Date.now() }]); }
    else { setShowTempDisclaimer(false); startNewChat(); }
    setIsTempChat(!isTempChat); setIsSidebarOpen(false);
  };

  const loadChat = (id) => {
    const chat = chatHistory.find(c => c.id === id);
    if (chat) { setMessages(chat.msgs); setActiveChatId(id); setIsTempChat(false); setIsSidebarOpen(false); }
  };

  const deleteChat = (id, e) => {
    e.stopPropagation(); setChatHistory(prev => prev.filter(c => c.id !== id));
  ...
  const handleSend = async () => {
    ...
    } catch { setMessages(prev => prev.filter(m => m.text !== '_thinking_').concat({ role: 'ai', text: "⚠️ Connection Failed.", id: Date.now() })); }
    setIsTyping(false);
  };
  ...
  const theme = {
    ...
    sidebarBg: isDarkMode ? 'bg-[#111111]/95' : 'bg-white/95',
    ...
    <>
      <style dangerouslySetInnerHTML={{__html: `
        * { margin: 0; padding: 0; box-sizing: border-box; }
        html, body, #root { width: 100vw; height: 100dvh; overflow: hidden; background-color: ${isDarkMode ? '#0a0a0a' : '#eef2f6'}; }
      ...
      <div className={`fixed inset-0 flex overflow-hidden ${theme.textPrimary} font-sans`}>
        ...
        <div className={`fixed md:relative w-[280px] sm:w-[320px] h-full flex-shrink-0 ${theme.sidebarBg} backdrop-blur-3xl flex flex-col border-r ${theme.sidebarBorder} z-50 transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
          <div className="p-5 space-y-4 flex-shrink-0">
            <div className="flex items-center justify-between">
              <h1 className={`text-xl font-extrabold tracking-tighter flex items-center gap-2 ${theme.textPrimary}`}>
                 <img src={logoImg} alt="Logo" className="w-10 h-10 rounded-xl object-cover border border-white/10" />
                 Elite AI <span className={`text-[10px] uppercase font-mono px-2 py-0.5 rounded-md text-white shadow-md ${accent.bg}`}>v3.0</span>
              </h1>
              <button onClick={() => setIsSidebarOpen(false)} className="md:hidden p-2 opacity-50"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M18 6L6 18M6 6l12 12"/></svg></button>
            </div>
            <button onClick={startNewChat} className={`w-full flex items-center gap-3 rounded-xl p-3.5 text-sm font-bold transition border ${theme.sidebarBorder} ${theme.sidebarHover} shadow-sm`}>+ New Conversation</button>
          ...
          <div className="flex-1 overflow-y-auto px-4 custom-scrollbar">
            {user ? chatHistory.filter(c => c.title.toLowerCase().includes(searchQuery.toLowerCase())).map(chat => (
              <div key={chat.id} onClick={() => loadChat(chat.id)} className={`p-3 rounded-xl cursor-pointer text-sm mb-1 transition flex items-center gap-3 ${activeChatId === chat.id ? theme.activeHistory : `${theme.sidebarHover} ${theme.textSecondary}`}`}>
          ...
          <div className="p-4 border-t border-white/5">
             {!user ? <button onClick={() => setShowAuthModal(true)} className={`w-full ${isDarkMode ? 'bg-white text-black' : 'bg-slate-900 text-white'} font-extrabold py-3.5 rounded-xl text-sm shadow-xl`}>Sign In</button> : 
               <button onClick={() => setShowProfileSettings(!showProfileSettings)} className="w-full flex items-center gap-4 p-2.5 rounded-2xl hover:bg-white/5 transition relative">
                  <div className={`w-10 h-10 rounded-full ${accent.bg} text-white flex items-center justify-center font-extrabold text-sm shadow-md`}>{user.email.charAt(0).toUpperCase()}</div>
                  <div className="flex-1 text-left truncate text-sm font-bold">{user.email}</div>
               </button>}
          </div>
        </div>

        {/* --- 💬 MAIN CHAT AREA (Flexbox Fix for Scrolling) --- */}
        <div className="flex-1 flex flex-col h-full relative z-10 min-w-0 overflow-hidden">
          ...
          {/* 🛑 CHAT LIST: Flex-1 ensures it fills the gap and SCROLLS 🛑 */}
          <div ref={chatContainerRef} className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-10 space-y-6">
          ...
          {/* 🛑 INPUT AREA: Fixed at bottom with Flex 🛑 */}
          <div className={`p-4 md:p-8 bg-gradient-to-t ${theme.inputAreaGradient} flex-shrink-0 z-20`}>
      ...
      {showAuthModal && (
         <div className="fixed inset-0 bg-black/80 backdrop-blur-2xl z-[200] flex items-center justify-center p-4 overflow-y-auto">
           <div className={`w-full max-w-md my-auto ${isDarkMode ? 'bg-[#111111]/95' : 'bg-white/95'} p-8 rounded-[2rem] border ${theme.sidebarBorder} shadow-2xl relative animate-slide-up`}>
              ...
              <button onClick={() => setShowAuthModal(false)} className="w-full mt-4 text-[10px] font-bold opacity-30 uppercase">Close</button>
           </div>
         </div>
      )}
    </>
